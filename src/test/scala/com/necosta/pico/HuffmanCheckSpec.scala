...
class HuffmanCheckSpec extends Properties("Huffman") {
  ...
  property("sum weight for forks of forks") = forAll(Gen.alphaChar, Gen.choose(0, maxWeight), Gen.choose(0, maxWeight)) {
    (char, weight1, weight2) =>
  ...
  property("keep chars for leaves") = forAll(Gen.alphaChar) {
    char =>
      val leaf = Leaf(char, 1)
      getChars(leaf) == List(char)
  }

  property("keep chars for forks") = forAll(Gen.alphaChar, Gen.alphaChar) {
    (char1, char2) =>
      val fork = Fork(Leaf(char1, 1), Leaf(char2, 1))
      getChars(fork) == List(char1, char2)
  }

  property("merge two leaves") = forAll(Gen.alphaChar) {
    char =>
      val leaf = Leaf(char, 1)
      val result = mergeTrees(leaf, leaf)
      result.left == leaf && result.right == leaf && getChars(result) == List(char, char) && getWeight(result) == leaf.weight * 2
  }

  property("merge two forks") = forAll(Gen.alphaChar, Gen.alphaChar,
    Gen.choose(0, maxWeight), Gen.choose(0, maxWeight), Gen.choose(0, maxWeight)) {
    (char1, char2, weight1, weight2, weight3) =>
      ...
      result.left == fork1 && result.right == fork2 && getChars(result) == List(char1, char2, char2, char1) && getWeight(result) == totalWeight
  }
}
